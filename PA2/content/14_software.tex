...
\chapter{Software}\label{ch:software_pa2}
	Während der \gls{ac:pa2} wurden die Software-Designs der \gls{ac:pa1} implementiert und bei Bedarf ergänzt. Es ist anzumerken, dass die entsprechenden Kapitel in der Dokumentation der \gls{ac:pa1} nicht aktualisiert wurden und deshalb veraltete Fakten beinhalten können.
	%
	%
	%System
	\section{System}\label{s:system}
		Das komplette System gliedert sich mehrere Komponente. Folgend wird detailliert auf einzelnen Aspekte eingegangen. Dabei wird Top $\rightarrow$ Bottom vorgegangen und nur der kleine Roboter betrachtet\footnote{Zum Zeitpunkt der Dokmentation war der grosse Roboter noch nicht betriebsbereit. Die Software wird aber über weite Bereiche indentisch ausfallen.}.
		%
		%
		\subsection{Roboter}
			...
			\begin{itemize}
				...
				\begin{description}
					...
					\item[Ultraschall Navigation]
						Ein auf Ultraschall basierendes Positionierungssytem verschafft uns eine absolute Position während eines Matchs.
					%
					...
					\item[Ultraschall Naherkennung]
						Um im nahen Umfeld des Roboters Gegenstände (auch Gegner) genau zu erkenne wird ein einfacher Ultraschallsensor verbaut. Er liefert ab einer bestimmten Distanz\footnote{aktuell 20 cm} genau Abstandsinformationen.
					%
					...
					\item[Taster Fresko 1, 2 \& Wand]
						Um verifizieren zu können ob ein Fresko anklebt oder die Wand erreicht wurde, werden drei einfache Hubtaster eingesetzt.
					%
					\item[Taster Schlüssel]
						Das Reglement schreibt vor, dass die Roboter mit dem Herausziehen einer Schnur gestartet werden müssen. In unserem Fall wird mit Hilfe der Schnur ein Schlüssel aus seiner Verankerung verschoben, die wiederum einen Taster betätigt.
					%
				...
				\begin{description}
					\item[Motor links/rechts]
						Sind zuständig für die Fortbewegung des Roboters.
					%
					\item[Servo Mammut]
						Betätigt den Mechanismus zum abfeuern der Bälle.
					%
					\item[Servo Fresko]
		...
		\subsection{Systemknoten}\label{ss:systemknoten}
			...
			\begin{description}
				\item[Kernknoten, \acrfull{ac:kernteam}]
					Dem Kernknoten unterliegt die Steuerung und Kontrolle des gesamten Systems. Weiter übernimmt er die Ansteuerung der spielaufgabenrelevanten Aktoren und Sensoren. Es ist Aufgabe des \gls{ac:kernteam} für seine Funktionalität zu sorgen. In den folgenden Abschnitten \ref{ss:kernknoten} bis \ref{ss:match} wird daher näher auf seinen Aufbau eingegangen.
				%
				...
				\item[Naviagtionsknoten, \acrfull{ac:navigationsteam}]
					Der Naviagtionsknote verschafft dem System Informationen über die aktuelle absolute Position des Systems. Ausserdem übernimmt es die Kommunikation mit den übrigen Robotern auf dem Spielfeld und tauscht mit ihnen Positionsinformationen aus.
			\end{description}
			%
		...
		\subsection{Kernknoten}\label{ss:kernknoten}
			...
			\begin{description}
				...
				\item[\gls{ac:elp} Task]
					...
					\end{itemize}
					Die Befehle sind dem Anhang H entnommen. Die Periodizität muss eventuell noch an die realen Gegebenheiten angepasst werden.
				%
				\item[Default Task]
					Für die einfache Visualisierung des \gls{g:rtos}-Zustandes wird der Default Task eingesetzt. Wenn das System ohne Einschränkungen läuft, so wird dies über ein Lichtspiel angezeigt. Im Falle eines Stackoverflows oder ähnlichen Misszuständen wird das Lichtspiel angehalten.
				%
				\item[Knoten Task]
					Im Knoten Task werden die einzelnen Aufgaben-Knoten, falls vom System Task gewünscht, der Spielstrategie (siehe dazu \cref{s:speilstrategie}) abgearbeitet. Dies beinhaltet die Steuerung von Aktoren unter Berücksichtigung von Sensorwerten. Weiter muss eine Kommunikation mit dem Rangfinder Task bestehen um mögliche Hindernisse während der Ausführung zu erkennen.
				%
				\item[Rangfinder Task]
					Für die Auswertung der Naherkennung ist der Rangfinder Task zuständig. Als Informationsquelle dienen ihm die zwei Ultraschallsensoren (hinten/vorne) und vier Infrarotschalter. Die Kommunikation zum Knoten Task erfolgt einerseits mit Hilfe einer Semaphore und andererseits mit globalen Zustandsflags.
				%
				\item[System Task]
					Die Abläufe der kompletten Applikation werden mit Hilfe des System Task gesteuert. Weiter ermöglicht er auch Benutzerinteraktionen durch entsprechende Taster (Schlüssel/Notstopp) und Anzeige. Für detaillierte Information zu den einzelnen Systemzuständen empfiehlt sich der \cref{ss:systemzustaende}.
				%
				\item[Timer Task]
					Eine Spielrunde dauert immer 90 Sekunden (+ 5 Sekunden Funny Action) \cite[]{lit:eurobot}. Sollte diese Zeit nicht eingehalten werden, so hat dies Punktabzüge zur Folge. Um diesen Fall zu vermeiden wird der Timer Task eingesetzt. Seine Aufgabe besteht darin die Spielzeit zu überwachen und bei Ablauf den Roboter zu stoppen (sollte dies nicht bereits der Fall sein).
			\end{description}
			%
			...
			\subsubsection{Task-Prioritäten}
				...
				%
				Die Priorität jedes spezifischen Tasks ist in der \cref{tab:prioritaeten} aufgeführt. Je höher die Zahl, desto höher ist auch die Priorität.
				%
				\begin{table}[htbp]
				    \centering
				    \begin{tabular}{|l|l|} 
	...
	\section{Bedieneinheit}\label{s:sw_bedieneinheit}
		...
		\end{itemize}
		%
		Dabei dient die display-Datei als Bibliotheksdatei (lib) und die RoboSetup-Datei als System-Datei (application/system). In der letztgenannten befindet sich die Menü-Steuerung um die gewünschten Spiel-spezifischen Initialisierungen einzustellen.
		\par
		...
		%
		\subsection{SPI-\"Ubertragung}\label{ss:spi}
			Ursprünglich wurde eine weitere Bibliotheksdatei (spi.c/.h) genutzt. Sie beinhaltet eine SPI-Initialisierungsfunktion (initSPI()) und eine Send-Byte-Funktion (SPI\_send\_byte()), die mit den von stm32f4 gegebenen SPI-Funktionen realisiert wurde.
			...
			\par
			Bei den ersten Tests tauchte jedoch das Problem auf, dass die eingestellte Baudrate zu hoch war und daher mit dem Oszilloskop keine erkennbare Byte-Übertragung festgestellt werden konnte. Folge dessen ist darauf zu achten, dass der SPI\_BaudRatePrescaler gross genug gewählt wird, damit die Frequenz dem Datenblatt des Controllers des Displays entsprechend kleiner als 300 kHz ist. Er berechnet sich mit der Formel:
			\par
			$Baudrate = \frac{(APB2 frequency (=84MHz))}{Prescaler}$
			\par
			Verwendet wurde in einem zweiten Versuch der maximal mögliche Prescaler von 256, was eine Baudrate von 328'125 Hz zur Folge hat.\\
			Trotz dieser (Fehl-)Konfiguration funktionierte das erste Display. Das zweite für den grossen Roboter jedoch nicht. Da das Erste so weit zuverlässig lief, wurde der Fehler in der HW gesucht. Nach aufwändiger Fehlersuche und der Mithilfe eines weiteren Kernteammitglieds (Simon) hat man die Fehlerquelle dennoch auf die SW begrenzen können. Dabei hat man festgestellt, dass nebst der Frequenz auch das RS-Pin nicht ganz korrekt gesetzt wurden. Es wurde nämlich jeweils zu beginn einer \"Ubertragung eines Instruktionsbytes gesetzt und nicht wie gemäss dem Datenblatt des Display-Controllers (s. 47) nur über dem achten Bit (Abbildung \ref{pic:timing}).
			\par
			\image{content/image/Bedienpanel/timing_sequenz.PNG}{scale=1}{htbp}[SPI timing sequenz][pic:timing]
			%
			Versuchsweise hat man ausserdem die eingestellte Übertragungsfrequenz (mit einem Interrupt) auf 100 kHz begrenzt. Dieser Wert gilt z.B. für das im Kapitel \ref{sss:NHD} beschriebene Display (NHD-0216K3Z) als Maximalwert. Umgesetzt wurde dies mit einem Timerinterrupt in der display.c-Datei, in dessen Handler schlussendlich die gesamte Datenübertragung umgesetzt wurde.\\
			Mit diesen Massnahmen funktionierte auch das zweite Display so weit einwandfrei.
			%
		...
		\subsection{RoboSetup}\label{ss:robosetup}
			...
			Es wurde versucht ein möglichst modularer Aufbau umzusetzen, damit wahlweise weitere Menüinhalte (Konfigurationen) hinzugefügt werden können. Erreicht wurde dies mit dem \texttt{typdef struct menu\_t} (Abbildung \ref{pic:menu_t}). Pro benötigte Konfiguration (Menüstate) muss somit zuerst je eine menu\_t-Struktur mit den gewünschten Initialisierungswerten erstellt werden.
			\par
			...
			%
			Beim Aufruf der menu\_handler()-Funktion muss nebst dem momentan aktuellen Menü (menu\_t) die auswählbaren Resultate übergeben werden. Ausserdem muss zwischen zwei Menütypen unterschieden werden: SELECTION\_MENU und ADJUSTING\_MENU.
			\par
	...
	\section{Spielstrategie}\label{s:speilstrategie}
		...
		\subsection{\gls{g:tsp}}\label{ss:tsp}
			...
			\subsubsection{MST-\gls{g:heuristik}}
				...
				\begin{enumerate}
					...
					\item Im Eulerkreis alle Kanten entfernen, die bereits zu einem besuchten Knoten führen und ersetze sie durch eine Kante zu einem Knoten, der im Eulerkreis an nächster Stelle kommt (ausser Startknoten). 
				\end{enumerate}	
		...
		\subsection{Konzept}\label{ss:konzept}
			...
			\subsubsection{Knotengewichtung}
				...
				%
				\image{content/image/14_node_arrive}{scale=0.4}{htbp}[Anfahrtsrichtung-Gewichtung (optimale Anfahrt aus Norden)][pic:anfahrt] 
				%
				...
				%
				\paragraph{Totales Knotengewicht}Das totale Gewicht eines Knoten berechnet sich anschliessend wie in \cref{eq:w_node} ersichtlich. Je tiefer das Gewicht ausfällt, dessto höher ist der Wert des Knoten.
				%
				\formula{
	...
	%
	%Spielaufgaben
	\section{Spielaufgaben}\label{s:spielaufgaben}
		Zum erledigen der Spielaufgaben ist der Task \texttt{vNodeTask} zuständig. Wie in Abb. \ref{pic:match} ersichtlich, wird dessen Task-Zustand vom System-Task auf \textit{Running} gesetzt sobald eine neue Node bearbeitet werden muss. Der Task ist in zwei Teile unterteilt, das Anfahren der Node und das Behandeln der Node.
		%
		\subsection{gotoNode}\label{ss:gotonode}
			Die Funktion \texttt{gotoNode} wird zu Beginn des Taskablauf aufgerufen. Als Parameter werden die Node-Informationen, sowie Informationen auf den aktuellen Spielzustand mitgegeben. Die Spielzustands-Informationen beinhalten Angaben wie die eigenen Positionen und Winkel, sowie die Positionen der Gegner.
			%
			\par Als Erstes wird der Rangefinder-Task in den \textit{Running}-State gebracht, damit die gesamte Fahrt überwacht und ein möglicher Zusammenstoss verhindert werden kann. Danach kann gleich losgefahren werden. Dies wird veranlasst, indem dem Antriebssystem via CAN-Message die Position des Nodes angegeben wird. Daraufhin liefert das Antriebssystem auf Anfrage die geschätzte Fahrdauer. Diese wird sogleich als Angabe benutzt wie lange versucht werden soll vom Rangefinder-Task eine Semaphore zu erhalten. Ist die Zeit abgelaufen ohne die Semaphore erhalten zu haben, so wird der Vorgang wiederholt bis die geschätzte Zeit gleich Null ist. Dies bedeutet, dass der Roboter das Ziel erreicht hat und nun die Spielaufgabe behandeln kann.
			%
			\par Übergibt jedoch der Rangefinder-Task während der Fahrtzeit die Semaphore, so bedeutet dies, dass ein Hindernis erkannt wurde. Nun muss überprüft werden, ob ein Notbremsung durchgeführt werden muss oder nicht. Dazu überprüft er zuerst ob die Distanz zum Gegner innerhalb der Rangefinder-Reichweite ist. Ist dies der Fall, so wird die die Position des Gegners relativ zur eigenen Position und Winkel berechnet. Befindet sich ein Gegnerroboter im Bereich vor dem eigenen Roboter, so wird einen Notbremsung ausgelöst. Ist ein Gegnerroboter in einem anderen Bereich, so besteht keine Gefahr eines Zusammenstosses und es kann weitergefahren werden. Durch diese Differenzierung wird beim Erkennen eines Feuer-Spielsteins oder einer Seitenwand nicht unnötig eine Notbremsung durchgeführt, sondern nur wenn wirklich ein Zusammenstoss mit dem Gegner bevorsteht. Zum Stand der Dokumentation ist diese Kalkulation jedoch noch nicht ausreichend ausgearbeitet und erfordert weitere Tests.
			%
			\clearpage
			\image{content/image/14_node_rangefinder}{scale=0.5}{htbp}[Ablauf des Node-Tasks und Kommunikation mit dem Rangefinder-Task][pic:node_rangefinder]
			\clearpage
			%
		\subsection{Bearbeitung der Spielaufgabe}\label{ss:doNode}
			Nach dem die Node angefahren wurde, ruft der Node-Task mittels Funktionspointer die zur Node gehörende Funktion auf. Die Funktionen zum bearbeiten der Spielaufgaben sind \texttt{doMammothNode}, \texttt{doFrescoNode} und \texttt{doFireNode}, wobei letztere die vom grossen Roboter behandelt wird zum Stand der Dokumentation noch nicht implementiert ist.
			%
			\par Nach der Bearbeitung einer Node wird der Zustand von der jeweiligen Funktion geändert. Folgende Möglichkeiten sind definiert:
			%
			\begin{description}[leftmargin=!, noitemsep, labelwidth=\widthof{\texttt{NODE\_FINISH\_SUCCESS}}]
				\item[\texttt{NODE\_UNDONE}] Node ist bereit, wurde also noch nicht bearbeitet.
				\item[\texttt{NODE\_FINISH\_SUCCESS}] Node wurde erfolgreich bearbeitet.
				\item[\texttt{NODE\_FINISH\_ERROR}] Node wurde bearbeitet, konnte aber nicht komplett erledigt werden.
				\item[\texttt{GOTO\_CAN\_ERROR}] Das Antriebssystem konnte nicht angesprochen werden. Das Setzen dieses Status hat zur Folge, dass das System in den Error-Zustand wechselt!
			\end{description}
			%
			\subsubsection{doMammothNode}\label{sss:doMammothNode}
				Die Behandlung der Mammut-Node besteht aus dem einmaligen Schleudern von zwei geladenen Bällen. Dafür müssen zwei Servomotoren angesteuert werden, einer zum Separieren/Laden der Bälle (Separations-Servo) und einer zum Zurückziehen des Spannbügels (Launcher-Servo), wodurch die Bälle aus dem Roboter katapultiert werden werden. Bei beiden Servos handelt es sich um digitale Varianten. Jedoch ist es nur beim Separations-Servo möglich vorgängig eine Geschwindigkeit einzustellen. Aus diesem Grund wird die Position des Launcher-Servos jeweils Schritt für Schritt in kleinen Abschnitten angefahren.
				%
				\par Die Funktion \texttt{doMammothNode} wird mehrmals für verschiedene Mammut-Nodes aufgerufen. Deshalb wird zu Beginn zuerst überprüft, ob es nötig ist zwei neue Bälle bereit zu machen (vereinzeln). Beim allerersten Aufruf der Funktion ist dies nicht nötig, da die Bälle vom Team schon bereit gemacht wurden. Bei weiteren Aufrufen der Funktion kommt es darauf an, ob im letzten Durchlauf die Vereinzelung durchgeführt werden konnte. Es kann nämlich sein, dass der benötigte Platz nicht verfügbar war, worauf entschieden wurde mit der Separation noch zu warten. Ist es in einer bestimmten Frist immer noch nicht möglich die Bälle zu vereinzeln, so wird die Funktion beendet und es wird der Status \texttt{NODE\_FINISH\_ERROR} für die Node gesetzt.
				%
				\par Ist die Vereinzelung aber gelungen, so wird der Launcher-Servo nach vorne und die Vereinzelung wieder rein bewegt. Jetzt ist der Roboter bereit für den Wurf. Dieser wird ausgelöst, indem der Launcher-Servo nach hinten bewegt wird. Danach kann er gleich wieder nach vorne bewegt werden und es wird in einer bestimmten Frist versucht die nächsten Bälle zu vereinzeln. Je nach dem ob dies gelungen ist, wird eine Variable gesetzt, mit der beim nächsten Aufruf der Funktion überprüft werden kann ob noch eine Separation nötig ist.
				%
			\subsubsection{doFrescoNode}\label{sss:doFrescoNode}
				Für die Bearbeitung des Fresko-Nodes wird nur ein Servomotor benötigt. Dabei handelt es sich um den selben wie für die Vereinzelung. Durch ausfahren der Vereinzelung wird auch das Panel mit den Freskos hinaus bewegt. Da es jedoch in dieser Anwendung nötig ist damit Kontakt zur Spielfeldwand herzustellen, darf dies nicht so schnell gehen wie eine Vereinzelung der Bälle. Während der Verwendung des Servos ist es aber nicht einfach möglich die eingespeicherte Geschwindigkeit zu ändern. Aus diesem Grund wird auch in diesem Fall die Servoposition Schritt für Schritt in kleinen Abschnitten angefahren.
				%
				\par In dieser Schlaufe wird auch gleich überprüft, ob der Sensor der sich vorne am Panel befindet die Wand detektiert hat. Falls dem so ist, wird das Freskopanel wieder rein gefahren. Auch dies darf nicht zu schnell geschehen, damit die Freskos gut an der Velcro-Wand kleben bleiben.
				%
				\par Nach dem das Panel wieder eingefahren wurde wird überprüft, ob sich die Freskos auch vom Panel gelöst haben und demnach an der Wand kleben. Dafür sind bei beiden Freskos im Panel Sensoren vorhanden. Sind nicht beide Freskos vom Panel gelöst worden, so wird der ganze Vorgang wiederholt (maximal so oft wie durch \texttt{FRESCO\_MAX\_RETRIES} definiert). Befindet sich zum Schluss noch immer mindestens ein Fresko im Roboter so wird der Status \texttt{NODE\_FINISH\_ERROR} für die Node gesetzt und die Funktion beendet.
				%
			%
			\clearpage
			\image{content/image/14_donode}{scale=0.6}{htbp}[Bearbeitung der Spielaufgaben][pic:donode]
			\clearpage
