...
\chapter{Software}\label{ch:software_pa2}
	...
	\section{Bedieneinheit}\label{s:sw_bedieneinheit}
		...
		\subsection{SPI}\label{ss:spi}
			Diese Bibliotheksdatei wurden schlussendlich nicht mehr genutzt, da die Datenübertragung manuell in der display-Datei programmiert wurde. Sie beinhaltet jedoch eine SPI-Initialisierungsfunktion (initSPI()) und eine Send-Byte-Funktion (SPI\_send\_byte()), die mit den von stm32f4 gegebenen SPI-Funktionen realisiert wurde.\\
			...
			\par
			Bei den ersten Tests tauchte das Problem auf, dass die eingestellte Baudrate zu hoch war und daher mit dem Oszilloskop keine erkennbare Byte-Übertragung festgestellt werden konnte. Es ist daher darauf zu achten, dass der SPI\_BaudRatePrescaler gross genug gewählt wird, damit die Frequenz dem Datenblatt des Controllers des Displays entsprechend kleiner als 300 kHz ist. Er berechnet sich mit der Formel:
			\par
		...
		\subsection{Display}\label{ss:display}
			...
			\par
			Bei der Übertragung eines Instruktionsbytes ist ausserdem zu beachten, dass das RS-Pin gemäss dem Datenblatt des Display-Controllers (s. 47) lediglich beim Bit 8 gesetzt wird.
			\par
